...
function buildPixPayload(amountCents, txid) {
  ...
  const merchantName = sanitizeAscii(env.pix.merchantName, 25) || 'ACARAJE 3 IRMAS';
...
async function buildPixQrCodeDataUrl(payload) {
  ...
  // é só a representação visual do payload. Não é persistido em
  // lugar nenhum: pode ser regerado a qualquer momento a partir do
  // mesmo payload (que esse sim fica salvo em payments.pix_payload).
